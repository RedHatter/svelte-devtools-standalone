...
<Button on:click={() => (isOpen = true)}>
  ...
  {#if isOpen}
    <div on:click|stopPropagation={() => (isOpen = false)} />
    <ul>
      <span />
      <li
        class:checked={$visibility.component}
        on:click={() => ($visibility.component = !$visibility.component)}
      >
        Components
      </li>
      <li
        class:checked={$visibility.element}
        on:click={() => ($visibility.element = !$visibility.element)}
      >
        Elements
      </li>
      <li
        class:checked={$visibility.block}
        on:click={() => ($visibility.block = !$visibility.block)}
      >
        Blocks
      </li>
      <li
        class:checked={$visibility.slot}
        on:click={() => ($visibility.slot = !$visibility.slot)}
      >
        Slots
      </li>
      <li
        class:checked={$visibility.anchor}
        on:click={() => ($visibility.anchor = !$visibility.anchor)}
      >
        Anchors
      </li>
      <li
        class:checked={$visibility.text}
        on:click={() => ($visibility.text = !$visibility.text)}
      >
        Text
      </li>
    </ul>
